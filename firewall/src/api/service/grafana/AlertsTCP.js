...
module.exports = {
  ...
  async parseAlerts(alertMessage) {
    ...
    while (match = regex.exec(alertMessage)) {
      const network_transport = match[2].replace(' ', '_');
      const destination_ip = match[1].replace(' ', '_');
  ...
  async filtrarNovosItens(req) {
    ...
    dadosPersistidos.forEach((objPersistido) => {
      const objetoRecebido = dadosRecebidos.find(
        (objRecebido) => objRecebido.network_transport === objPersistido.network_transport
    ...
    const dadosAtualizados = novosItens.concat(
      dadosRecebidos.filter(
        (obj) => !novosItens.some(
          (dado) => dado.network_transport === obj.network_transport
