...
/*
func (hs *HTTPServer) getFirewallNode(c *models.ReqContext) *dtos.NavLink {
	// Only set login if it's different from the name
	var login string
	if c.SignedInUser.Login != c.SignedInUser.NameOrFallback() {
		login = c.SignedInUser.Login
	...
	children := []*dtos.NavLink{
		{
			Text: "Create Rule", Id: "firewall-settings", Url: hs.Cfg.AppSubURL + "/firewall", Icon: "sliders-v-alt",
		},
	}

	if setting.AddChangePasswordLink() {
		children = append(children, &dtos.NavLink{
			Text: "List Rules", Id: "list-rules", Url: hs.Cfg.AppSubURL + "/firewall/list",
			Icon: "lock",
		})
	}
...
func (hs *HTTPServer) getNavTree(c *models.ReqContext, hasEditPerm bool) ([]*dtos.NavLink, error) {
	...
	firewallChildNavs := []*dtos.NavLink{
		{Text: "Firewall rules", Id: "firewall-rules", Url: hs.Cfg.AppSubURL + "/firewall/", Icon: "plus-circle"},
		{Text: "Firewall list rules", Id: "firewall-list-rules", Url: hs.Cfg.AppSubURL + "/firewall/list", Icon: "list-ul"},
	}
	...
	navTree = append(navTree, &dtos.NavLink{
		...
		Id:         "firewall",
		Icon:       "fire",
		Url:        hs.Cfg.AppSubURL + "/firewall",
		Children:   firewallChildNavs,
		SortWeight: dtos.WeightAlerting,
	})
